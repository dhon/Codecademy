-- PRO | Aggregate Functions | Fake Apps | Freeform Project

-- 1. Return the total number of apps in the table fake_apps.
SELECT COUNT(*) FROM fake_apps;

-- 2. Return the name, category, and price of the app that has been downloaded the least amount of times.
SELECT name, category, price FROM fake_apps
...
-- 13. Return the sum of the total number of downloads for apps that belong to the Music category.
SELECT SUM(downloads) FROM fake_apps
...
SELECT category, COUNT(category) FROM fake_apps
GROUP BY category;
